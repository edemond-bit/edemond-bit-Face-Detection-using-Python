...
(h, w) = image.shape[:2]
blob = cv2.dnn.blobFromImage(image, 1.3, (300, 300),
	(104.0, 177.0, 123.0))
...
for i in range(0, detections.shape[2]):
	...
	if confidence > args["confidence"]:
		...
		# make predictions on gender and find the gender bucket with
		# the largest corresponding probability
		genderNet.setInput(faceBlob)
		preds_G = genderNet.forward()
		j = preds_G[0].argmax()
		gender = genderList[j]
		genderConfidence = preds_G[0][j]		

		# display the predicted age and gender to our terminal
		text = "{}: {:.2f}% {}: {:.2f}%".format(age, ageConfidence * 100, gender, genderConfidence * 100)
